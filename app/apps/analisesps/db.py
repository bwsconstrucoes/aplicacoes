# -*- coding: utf-8 -*-
"""
Camada de banco da Análise de SPs.

Mora no MESMO Postgres do ERP (a variável DATABASE_URL), mas dentro de um
schema separado chamado `analisesps`. Isso é proposital: o ERP já tem tabelas
com nomes genéricos (`titulos`, `rateios`, `categorias`), e este módulo tem a
sua própria noção de solicitação de pagamento. Schemas separados fazem as duas
conviverem sem renomear nada e sem nenhum risco de uma escrita daqui encostar
em dado do ERP.

A engine é PREGUIÇOSA, pela mesma razão do ERP (`erp/db/database.py`): nada de
banco acontece no import. Sem DATABASE_URL o módulo falha sozinho, na primeira
...
logger = logging.getLogger("analisesps.db")
...
def _recusar_producao_em_teste(url: str) -> None:
    ...
    Aconteceu de verdade durante a conversao do painel: um teste novo chamou a
    prestacao de contas sem dublar tudo, e o modulo tentou abrir o Postgres do
    Render. So nao deu em nada porque o usuario do banco nao tinha permissao de
    login. A trava veio de la e vale aqui pela mesma razao.
    ...
    if host not in {"localhost", "127.0.0.1", "::1"} or "teste" not in nome:
        raise RuntimeError(
            "O módulo tentou abrir um banco que NAO e de teste durante a suite "
...
def _montar_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL nao definida. O módulo guarda os dados no mesmo Postgres "
...
def obter_engine():
    ...
    Pool pequeno de proposito: a instancia tem 2 GB e divide com 15 modulos.

    O schema deste modulo e fixado como PARAMETRO DA CONEXAO (`options`), nao com
    ...
    if _engine is None:
        ...
        logger.info("Análise de SPs: engine criada (schema %s).", SCHEMA)
    return _engine
...
# --------------------------------------------------------------------------- 
# O codigo que le a planilha SPsBD foi escrito contra o sqlite3, com
# `conn.execute(...)` devolvendo cursor iteravel e placeholder `?`. Reescrever
# o SQL a mao seria trocar codigo rodando ha meses por codigo novo e nao
# testado. Este adaptador faz a ponte: a regra de negocio segue identica, so
# o banco por baixo muda.
...
def _em_lote(cursor, sql, seq_params):
    ...
    Sem isto, gravar 59 mil SPs seria 59 mil idas e voltas ate o banco —
...
class CursorCompat:
    ...
    Existe porque parte do codigo da sincronizacao pede `conn.cursor()` e depois
...
class ConexaoCompat:
    """Conexao Postgres com a interface do sqlite3 usada pela sincronizacao."""
    ...
    def executar_em_stream(self, sql, params=(), por_vez=2000):
        ...
        que o execute roda — as 59 mil SPs de uma vez, exatamente o que este
        modulo precisa evitar. Um cursor nomeado deixa as linhas no Postgres e
        ...
        nome = "analisesps_%d" % (id(self) ^ hash(sql) & 0xFFFFFF)
