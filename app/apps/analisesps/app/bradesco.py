# -*- coding: utf-8 -*-
"""
bradesco.py — lê a tela de autorização do Bradesco (colada como texto) e cruza com a
SPsBD (o DataFrame do app).

Portado de organizarPgtBradesco (boleto/conta de consumo) e organizarPgtBradescoPix
(Pix). Diferença: aqui o cruzamento é contra o df da SPsBD, não contra uma aba 'Lote'.

Regras:
- Pix: casa por CONTA + VALOR (exato) e desempata por similaridade de nome
  (Jaccard 0.65 + Levenshtein 0.35), confiança 90–100.
- BeeVale: pix cujo valor debitado pode ser o valor da SP OU 1,5% maior; tolerância ±0,02.
- Boleto: extrai código de barras + nº da SP + valor do texto; casa pela SP (col A) e/ou
  pelo código de barras (col AI).
- Conta de Consumo: extrai o convênio (≥30 dígitos) e casa pelo código de barras (AI).
"""
...
def cruzar_tudo(raw: str, df, foco_agendados: bool = True) -> dict:
    ...
    df_base = df[df.apply(_no_foco, axis=1)] if foco_agendados else df
